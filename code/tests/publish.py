...
def main():
    ...
    rpc.open('logs', 'logs', 'direct')
    response = rpc.dispatch('{"exchange":"logs", "exchange_type":"direct", "queue":"logs", "routing_key":"logs"}')
    print(" [.] Celery task: %s" % response)
    # Connect to localhost:5672 as guest with the password guest and virtual host "/" (%2F)
    publisher = LogPublisher('amqp://guest:guest@broker:5672/%2F?connection_attempts=3&heartbeat_interval=3600')
    try:
        publisher.run()
